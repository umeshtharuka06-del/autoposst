...
const TONE_GUIDES: Record<Tone, string> = {
  PROFESSIONAL:
    'Polished, credible, businesslike. No slang. Minimal emoji (max 1-2 subtle ones). Focus on value and trust.',
  FRIENDLY:
    'Warm, welcoming, conversational. Moderate emoji use. Speak like a helpful friend, not a brand.',
  GAMING:
    'High-energy, playful, meme-aware. Gamer vocabulary where natural (GG, level up, clutch). Bold emoji use.',
  LUXURY:
    'Elegant, aspirational, understated. Sensory language. Very sparse, refined emoji (✨ at most). Never salesy.',
  CASUAL:
    'Relaxed, everyday language. Short sentences. Light emoji. Feels like a personal post, not marketing.',
};
...
export function contentJsonSchema(): Record<string, unknown> {
